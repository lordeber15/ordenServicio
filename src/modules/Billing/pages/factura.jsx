...
const handleDescargarXml = async (id) => {
  try {
    ...
    downloadBlobResponse(res, `factura-${id}.xml`);
  } catch {
...
function Factura() {
  ...
  const handleEmitir = async (metodoPago) => {
    ...
    try {
      ...
      if (observado) toast("⚠ Factura aceptada con observaciones de SUNAT", { icon: "⚠️" });
      else if (aceptado) toast.success("✓ Factura aceptada por SUNAT");
      ...
      <div className="py-2 flex gap-2 flex-col">
        ...
        {resultado && (() => {
          ...
          const title = esperando
            ...
            : en_proceso
              ? "⏳ En proceso — revisa en Ventas del Día"
              : observado ? "⚠ Factura aceptada con observaciones"
              : success ? "✓ Factura aceptada" : "✗ Factura rechazada";
