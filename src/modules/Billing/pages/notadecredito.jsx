...
function NotaDeCredito() {
  ...
  const { data: series = [] } = useQuery({ queryKey: ["series", "07"], queryFn: () => getSeriesByTipo("07") });
  const { data: unidades = [] } = useQuery({ queryKey: ["unidades"], queryFn: getUnidades });
  const { data: comprobantes = [] } = useQuery({ queryKey: ["comprobantes"], queryFn: () => axiosURL.get("/comprobante").then(res => res.data) });
  ...
  const [comprobanteRefId, setComprobanteRefId] = useState("");
  const [tipoNotaId, setTipoNotaId] = useState("01");
  ...
  const handleEmitir = async () => {
    ...
    try {
      ...
      const cabRes = await createComprobante({
        ...
        comprobante_ref_id: comprobanteRefId ? parseInt(comprobanteRefId, 10) : null,
      });
      ...
      // 5. Firmar y encolar en SUNAT (202 inmediato)
      await toast.promise(
        emitirComprobante(comprobanteId),
        {
          loading: "Firmando y enviando a SUNAT…",
          success: "Nota en cola — esperando respuesta…",
          error: (e) => `Error SUNAT: ${e?.response?.data?.message || e.message}`,
        }
      );

      setResultado({ comprobante_id: comprobanteId, esperando: true });

      // 6. Polling hasta respuesta final de SUNAT (máx 30 s)
      ...
      if (observado)      toast("⚠ Nota de Crédito aceptada con observaciones de SUNAT", { icon: "⚠️" });
      else if (aceptado)  toast.success("✓ Nota de Crédito aceptada por SUNAT");
      ...
      toast.error(err?.response?.data?.message || err.message || "Error al emitir Nota de Crédito");
    } finally {
      setEmitiendo(false);
    }
  };
  ...
  return (
    <div className="px-4 md:px-12 py-4 w-full">
      <div className="flex justify-start gap-5 items-center">
        <Drawer />
        <div className="text-xl md:text-3xl font-black text-amber-600 dark:text-amber-500 transition-colors tracking-tight">Emitir Nota de Crédito</div>
        {fromTicket && (
      ...
      <div className="flex justify-center items-center w-full p-2 mt-6">
        <div className="grid grid-cols-1 md:grid-cols-2 w-full gap-6">
          ...
          <div className="p-6 flex justify-center items-center flex-col border-2 border-dashed border-gray-300 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-900 transition-colors shadow-sm">
            ...
            <div className="font-black text-2xl text-center text-amber-600 dark:text-amber-500 uppercase tracking-tighter mt-1">Nota de Crédito</div>
            <div className="flex px-4 py-2 bg-slate-100 dark:bg-slate-950 rounded-lg items-center mt-3 border dark:border-slate-800">
              {serie
                ? <span className="text-amber-600 dark:text-amber-500 font-mono font-bold tracking-widest">{serie.serie} — {String(serie.correlativo).padStart(8, "0")}</span>
                : <span className="text-red-500 text-sm font-bold">Sin serie de Nota de Crédito configurada</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="py-2 flex gap-2 flex-col">
        ...
        <div className="flex flex-col md:flex-row gap-4 mt-4 bg-slate-50 dark:bg-slate-900 border border-amber-200 dark:border-amber-900/50 p-4 rounded-xl shadow-inner">
          <div className="w-full md:w-1/3">
            ...
            <select id="nc-doc-ref" value={comprobanteRefId} onChange={(e) => {
                ...
                if (cRef) {
                  // Cargar ítems automáticamente si fuera necesario
                }
              }}
              ...
              {comprobantes.filter(c => ['01', '03'].includes(c.tipo_comprobante_id) && c.estado_sunat === 'AC').map(c => (
                <option key={c.id} value={c.id}>{c.serie}-{c.correlativo} (S/ {c.total})</option>
              ))}
            </select>
          </div>
          <div className="w-full md:w-1/3">
        ...
        {resultado && (() => {
          ...
          const title = esperando
            ...
            : observado ? "⚠ Nota aceptada con observaciones"
            : success ? "✓ Nota aceptada" : "✗ Nota rechazada";
        ...
        <div className="flex justify-center md:justify-end mt-8">
          <button onClick={handleEmitir} disabled={emitiendo || items.length === 0 || !comprobanteRefId}
            className="bg-amber-600 hover:bg-amber-500 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl p-4 w-full md:w-1/4 text-white cursor-pointer font-black uppercase tracking-[0.2em] shadow-xl transition-all hover:scale-105 active:scale-95 disabled:hover:scale-100">
            {emitiendo ? "Enviando..." : "Emitir Nota"}
          </button>
        </div>
      </div>
    </div>
